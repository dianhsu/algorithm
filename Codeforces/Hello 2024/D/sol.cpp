...
int main() {
  ...
  while (t--) {
    ...
    while (pq.size() > 1) {
      auto it = pq.top();
      pq.pop();
      auto real_position = dsu.leader(it.second);
      PII range = rg[real_position];
      if (range.first > 0) {
        int lp = dsu.leader(range.first - 1);
        if (arr[lp] == arr[real_position] or
            arr[lp] == arr[real_position] - 1) {
          dsu.merge(lp, real_position);
          if (arr[lp] == arr[real_position] - 1) {
            size[dsu.leader(lp)] = size[lp];
          } else {
            size[dsu.leader(lp)] += size[real_position];
          }
          rg[dsu.leader(lp)] = {rg[lp].first, range.second};
          arr[dsu.leader(lp)] = min(arr[real_position], arr[lp]);
          continue;
        }
      }
      if (range.second < n - 1) {
        int rp = dsu.leader(range.second + 1);
        if (arr[rp] == arr[real_position] or
            arr[rp] == arr[real_position] - 1) {
          dsu.merge(rp, real_position);
          if (arr[rp] == arr[real_position] - 1) {
            size[dsu.leader(rp)] = size[rp];
          } else {
            size[dsu.leader(rp)] += size[real_position];
          }
          rg[dsu.leader(rp)] = {range.first, rg[rp].second};
          arr[dsu.leader(rp)] = min(arr[real_position], arr[rp]);
          continue;
        }
      }
      ok = false;
    ...
    cout << (ok && arr[dsu.leader(0)] == 0 && size[dsu.leader(0)] == 1 ? "YES"
                                                                       : "NO")
         << endl;
  }
  return 0;
